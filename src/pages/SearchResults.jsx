  ...
  if (catError || userError)
    return (
      <ErrorBox>
        ...
        {catError || userError || 'Errore nel caricamento'}
      </ErrorBox>
    )
  ...
  return (
    <section className={SECTION_STYLES}>
      ...
      <div className={GRID_STYLES} style={{ gridTemplateColumns: `repeat(auto-fit, 12rem)` }}>
        {userLoading || catLoading ? Array.from({ length: 4 }).map((_, i) => <CardSkeleton key={i} />) : filtered?.map((user) => <Card key={user.id} obj={user} img={user.avatar} title={user.firstName} description={user.bio} variant='hoverScale' />)}
      </div>
    </section>
  )
}
