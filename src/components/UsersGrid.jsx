    ...
    <div className={cn(GRID_STYLES, className)} style={gridTemplate}>
      {loading
        ...
        : users
            ?.slice(1, limit + 1)
            .map((user) => (
              <Card key={user.id} obj={user} img={user.avatar} title={user.firstName} description={user.bio} buttons={pending} variant='hoverScale' onAccept={() => confirmMatch({ user2Id: user.id })} onReject={() => cancelMatch(user.id)} />
            ))}
    </div>
  )
}
